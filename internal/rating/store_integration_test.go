...
func TestIntegration_C4ResolutionLadderConformsToOracle(t *testing.T) {
	...
	for i, c := range cases {
		if _, err := db.ExecContext(ctx,
			`INSERT INTO billing_event (request_id, auth_id, model, base_model, adapter, prompt_tokens, completion_tokens, event_ts)
			 VALUES ($1,'a',$2,$3,$4,$5,0,$6)`,
